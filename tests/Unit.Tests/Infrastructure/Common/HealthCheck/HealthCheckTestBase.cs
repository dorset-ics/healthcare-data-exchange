...
{
    private readonly IHttpClientFactory ClientFactory = Substitute.For<IHttpClientFactory>();
    private MockHealthCheck Sut { get; }

    private void SetupHttpClient(HttpStatusCode statusCode)
    {
        HttpClientMocker.SetupHttpClient(ClientFactory, statusCode);
    }

    public HealthCheckTestBase()
    {
        Sut = new MockHealthCheck(ClientFactory);
    }
