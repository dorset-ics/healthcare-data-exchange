...
public abstract class BaseApiTest(ITestOutputHelper outputHelper) : IDisposable
{
    ...
    protected Dictionary<MeshClientName, MeshClientsSettings> MeshSettings { get; } = ConfigurationLoader.GetMeshClientsSettings();
    private CommonMeshSettings CommonMeshSettings { get; } = ConfigurationLoader.GetCommonMeshSettings();
    ...
    protected MeshClient GetMeshClient(MeshClientName clientName)
    {
        var clientSettings = MeshSettings[clientName];
        return new MeshClient(new MeshConfiguration
        {
            MailboxId = clientSettings.MailboxId,
            Key = clientSettings.Key,
            Url = CommonMeshSettings.Url,
            MaxChunkSizeInMegabytes = CommonMeshSettings.MaxChunkSizeInMegabytes,
            Password = clientSettings.MailboxPassword
        });
    }
